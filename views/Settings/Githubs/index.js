...
const View = ({ pageInfo, queries, mutations }) => {
  ...
  const tableRows = queries.githubs.datas.edges.map(element => (
    <tr key={element.node._id}>
      <td>
        <Link href={`/settings/github/${element.node._id}`}>
          ...
          <Menu.Item
            ...
            onClick={async () => {
              if (
                ...
                await mutations.deleteGithub(element.node._id)
              }
            }}
          >
            Delete
    ...
    <Layout title={pageInfo.title}>
      <div className="space-y-3">
        ...
        <article className="bg-white">
          <Table
            loading={queries.githubs.isLoading}
            columns={['Label', 'Email', 'Created At', '']}
            rows={tableRows}
          />
        </article>
        <footer className="-mt-2 flex items-center justify-center">
          <Pagination
            ...
            total={queries.githubs.datas.pageInfo.totalPages}
          />
        </footer>
      </div>
      <Modal
