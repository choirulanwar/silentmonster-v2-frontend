...
const View = ({ pageInfo, queries, mutations }) => {
  ...
  const tableRows = queries.oauthKeys.datas.edges.map(element => (
    <tr key={element.node._id}>
      <td>
        <Link href={`/settings/oauth/${element.node._id}`}>
          ...
          <Menu.Item
            ...
            onClick={async () => {
              if (
                ...
                await mutations.deleteOAuthKey(element.node._id)
              }
            }}
          >
            Delete
    ...
    <Layout title={pageInfo.title}>
      <div className="space-y-3">
        ...
        <article className="bg-white">
          <Table
            loading={queries.oauthKeys.isLoading}
        ...
        <footer className="-mt-2 flex items-center justify-center">
          <Pagination
            ...
            total={queries.oauthKeys.datas.pageInfo.totalPages}
          />
        </footer>
      </div>
      <Modal
