...
const View = ({ pageInfo, queries, mutations }) => {
  ...
  const tableRows = queries.clouds.datas.edges.map(element => (
    <tr key={element.node._id}>
      <td>
        <Link href={`/settings/cloud/${element.node._id}`}>
          ...
          <Menu.Item
            ...
            onClick={async () => {
              if (
                ...
                await mutations.deleteCloud(element.node._id)
              }
            }}
          >
            Delete
    ...
    <Layout title={pageInfo.title}>
      <div className="space-y-3">
        ...
        <article className="bg-white">
          <Table
            loading={queries.clouds.isLoading}
        ...
        <footer className="-mt-2 flex items-center justify-center">
          <Pagination
            ...
            total={queries.clouds.datas.pageInfo.totalPages}
          />
        </footer>
      </div>
      <Modal
